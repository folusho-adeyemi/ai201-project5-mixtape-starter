...
def search_songs(query: str) -> list[dict]:
    ...
    """
    # No join to song_tags: the search matches only on title/artist, and tags
    # are loaded via the Song.tags relationship. Joining the tag association
    # table produced one row per tag, which is the source of duplicate results.
    results = (
