...
def get_friends_listening_now(user_id: str) -> list[dict]:
    ...
    "Listening now" only includes plays from the current calendar day (UTC),
    not a rolling 24-hour window — a play from yesterday evening should drop
    off the feed at midnight, not linger until the same time the next day.
    ...
    # Start of today (UTC): only plays from the current calendar day count.
